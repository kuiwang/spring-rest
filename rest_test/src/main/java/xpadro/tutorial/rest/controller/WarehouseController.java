...
public class WarehouseController {
    ...
    public @ResponseBody
    Warehouse getWarehouse(@PathVariable("warehouseId") int id) {
     ...
     * 
     * @param warehouseId The id of the warehouse where to add the new
     *        product
    ...
    public void addProduct(@PathVariable("warehouseId") int warehouseId,
            @RequestBody Product product, HttpServletRequest request, HttpServletResponse response) {

        warehouseRepository.addProduct(warehouseId, product);
        response.setHeader("Location", request.getRequestURL().append("/").append(product.getId())
                .toString());
    }
     ...
     * 
     * @param warehouseId The id of the warehouse where to remove the
     *        product
    ...
    public void removeProduct(@PathVariable("warehouseId") int warehouseId,
    ...
    public @ResponseBody
    Product getProduct(@PathVariable("warehouseId") int warehouseId,
            @PathVariable("productId") int productId) {
        return warehouseRepository.getProduct(warehouseId, productId);
    }

    /**
     * Handles ProductNotFoundException and returns a 404 response status
     * code
     */
